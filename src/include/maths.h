...
struct vec2_T
{
  ...
  float getmag() const { return sqrtf(pow(x, 2) + pow(y, 2)); }

  operator glm::vec4() { return glm::vec4(x, y, 1, 1.0); }

  operator glm::vec2() { return glm::vec2(x, y, 1.0); }
  ...
  vec2_T<T> operator+(T a) const { return vec2_T<T>(x + a, y + a); }
  ...
  T operator*(const vec2_T<T> &b) const { return x * b.x + y * b.y; }
  ...
  inline bool operator!=(const vec2_T<T> &in) const { return !operator==(in); }
...
struct vec3_T
{
  ...
  float getmag() const { return sqrtf(pow(x, 2) + pow(y, 2) + pow(z, 2)); }

  operator glm::vec4() { return glm::vec4(x, y, z, 1.0); }
  ...
  vec3_T<T> operator+(T a) const { return vec3_T<T>(x + a, y + a, z + a); }
  ...
  T operator*(const vec3_T<T> &b) const { return x * b.x + y * b.y + z * b.z; }
  ...
  inline bool operator!=(const vec3_T<T> &in) const { return !operator==(in); }
...
struct vec4_T
{
  ...
  template <class U>
  vec4_T(const vec3_T<U> &in, const T &iw = 1)
      : x(in.x), y(in.y), z(in.z), w(iw) {}
  ...
  vec4_T<T> operator-() const { return vec4_T<T>(-x, -y, -z, -w); }
  ...
  inline bool operator!=(const vec4_T<T> &in) const { return !operator==(in); }
...
public:
  ...
  vec3_T<Type> operator*(const vec3_T<Type> &in)
  {
    ...
    ret.y = operator()(1, 0) * in.x + operator()(1, 1) * in.y +
                                      operator()(1, 2) * in.z;
    ret.z = operator()(2, 0) * in.x + operator()(2, 1) * in.y +
                                      operator()(2, 2) * in.z;
    return ret;
  }
  ...
  vec4_T<Type> operator*(const vec4_T<Type> &in)
  {
    ...
    ret.z = operator()(2, 0) * in.x + operator()(2, 1) * in.y +
                                      operator()(2, 2) * in.z +
                                      operator()(2, 3) * in.w;
    ret.w = operator()(3, 0) * in.x + operator()(3, 1) * in.y +
                                      operator()(3, 2) * in.z +
                                      operator()(3, 3) * in.w;
    return ret;
  }
