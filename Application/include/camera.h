...
class camera {
  public:
    ...
    camera() : viewDir(0.0f, 0.0f, -1.0f), eye(0.0f, 0.0f, 20.0f), up(0.0f, 1.0f, 0.0f), pitchAngle(0) {
        checkCorrectness();
    }

    camera(const vec3 &pos, const vec3 &up, const vec3 &dir) : viewDir(dir), up(vec3::normalize(up)), eye(pos), pitchAngle(0) {
        checkCorrectness();
    }
